...
public class ShiroAuthService {
    ...
    private String sessionKey = SystemConstant.ADMIN_SESSION_KEY;
    private String cookieIdName = SystemConstant.ADMIN_COOKIE_ID_NAME;
    private String cookieTicketName = SystemConstant.ADMIN_COOKIE_TICKET_NAME;
    ...
    public String saveTicket(HttpServletRequest request, HttpServletResponse response, Integer id, Boolean adminFlag) {
        String ticket;
        if (systemProperties.getSso()) {
            ticket = getAndSaveTicket(id, adminFlag);
        } else {
    ...
    private String getAndSaveTicket(Integer id, Boolean adminFlag) {
        // 获取并保存 ticket
        String ticket = createTicket();
        JSONObject o = new JSONObject();
        o.put(cookieIdName, id);
        o.put(cookieTicketName, ticket);
        o.put("adminFlag", adminFlag);
    ...
    public String getTicket(String id) {
        try {
            String session = (String) redisCacheManager.hGet(sessionKey, id);
            if (StringUtils.isNoneEmpty(session)) {
                JSONObject o = JSONObject.parseObject(session);
                return o.getString(cookieTicketName);
            }
        } catch (Exception e) {
            return "";
        }
