...
//        List<JXNode> newJxNodes = doc.selN("//dl[@class='location']/dd/a/@href");
//        for(int j = 0, len = newJxNodes.size(); j < len; j++) {
//            String jxNode = newJxNodes.get(j).toString();
//            String provinceId = jxNode.substring(jxNode.indexOf("=") + 1, jxNode.length());
            ...
            //            String categoryUrl = "https://auction.jd.com/getAssetsList.html?childrenCateId=12762&paimaiStatus=2&page=";
    ...
    public void getPageData(String newUrl, String categoryName, CloseableHttpClient httpClient){
        try {
            ...
            for(int i = 0; i < jsonArray.size(); i++){
                ...
                if(oneByCode == null) {
                    String detailPageUrl = "https://mpaimai.jd.com/json/mobile/getProductbasicInfo.html?paimaiId=";
...
//            JdPmTransactionDataAssets oneByCode = jdPmTransactionDataDao.getOneByCode(id);  //
            ...
            String agencyUrl = "https://paimai.jd.com/json/current/queryVendorInfo.html?vendorId=" + vendorId;
            setAgencyInfo(agencyUrl, httpClient, jdPmTransactionData);

            //设置另外一个详情
            String bidListUrl = "https://paimai.jd.com/json/current/englishquery.html?paimaiId="+ id +"&skuId="+skuId+"&start=0&end=9";
            getAnotherDetailData(bidListUrl, httpClient, jdPmTransactionData, skuId);

            String likeListUrl = "https://paimai.jd.com/json/ensure/queryAccess?paimaiId=108498022";
    ...
    private void getBidList(CloseableHttpClient httpClient, String code, Integer intCount, String skuId) {
        ...
        try{
            for (int i = 0; i < intCount / 10 + 1; i++){
               String bidPageListUrl = "https://paimai.jd.com/json/current/englishquery.html?paimaiId="
