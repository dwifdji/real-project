package com._360pai.applet.aspact;
...
public class LogAspect extends AbstractLogAspect {
    ...
    protected String getApplicationName() {
        return SystemConstant.APPLICATION_NAME_APPLET;
    }
    ...
    public AccountInfo getAccountInfo(HttpServletRequest request) {
        ...
        if (cookies != null && cookies.length > 0) {
            for (Cookie cookie : cookies) {
                ...
                if (SystemConstant.APPLET_COOKIE_ACCOUNT_TYPE.equals(cookie.getName())) {
                    type = cookie.getValue();
                }
            }
        }
        ThreadLocalContextUtil.setValue(SystemConstant.APPLET_COOKIE_ACCOUNT_ID_NAME,cookieId);
        ThreadLocalContextUtil.setValue(SystemConstant.APPLET_COOKIE_ACCOUNT_TYPE,type);
