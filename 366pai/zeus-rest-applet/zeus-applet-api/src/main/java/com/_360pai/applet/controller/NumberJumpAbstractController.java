 ...
 */
@Controller
public class NumberJumpAbstractController {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(NumberJumpAbstractController.class);
    ...
    protected final Integer loadCurLoginId() {
        ...
        return accountBaseInfo.getExtBindId();
    }
    ...
    protected final AccountBaseInfo loadCurLoginAccountInfo() {
        ...
        if (cookies != null && cookies.length > 0) {
            for (Cookie cookie : cookies) {
                if (SystemConstant.CALCULATOR_COOKIE_ACCOUNT_ID_NAME.equals(cookie.getName())) {
            ...
            //仅是  account 用户
            AccountExtBindIVo resp = accountFacade.getNumberJumpExtBind(cookieId);
