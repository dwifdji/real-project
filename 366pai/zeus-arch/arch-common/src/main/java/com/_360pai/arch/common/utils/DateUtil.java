...
public class DateUtil {
     ...
     * 新增日期格式
     */
    public final static String DOT_YYYY_DATE_PATTERN = "yyyy.MM.dd";
     ...
     * 时分显示
     */
    public final static String NORM_TIME_FEN = "HH:mm";
     ...
     * 比较两个时间相差的天数（只比较日期部分）
     */
    public static Long differentDaysByDate(Date startDate, Date endDate) {
...
// 如果throw java.text.ParseException或者NullPointerException，就说明格式不对
     ...
     *
     */
    public static int differenceYear(Date bigDate, Date smallDate) {
        Calendar bef = Calendar.getInstance();
        Calendar aft = Calendar.getInstance();
        try{
            bef.setTime(bigDate);
            aft.setTime(smallDate);
        }catch (Exception e){
            e.printStackTrace();
        }

        int result = bef.get(Calendar.YEAR) - aft.get(Calendar.YEAR);
        return  result;
    }
...
//    }
     ...
     * 获取距今相差年份
     */
    public static int getYearMargin(Date date) {
        int y = 1;
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
     ...
     * @return
     */
    public static Date formatDay(Date date,String pattern) {
    ...
    public static String formatAmPm(Date date) {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("a hh:mm",Locale.CHINA);
