...
public class MathUtil {
     ...
     *
     */
    public static boolean scopeData(String amount, String area) {
        //为0直接返回
        if(BigDecimal.ZERO.compareTo(new BigDecimal(amount))==0||BigDecimal.ZERO.compareTo(new BigDecimal(area))==0){
            return true;
        }
        ...
        //均价
        BigDecimal average = MathUtil.divide(new BigDecimal(amount),new BigDecimal(area));


        //当均价大于 20 万时 丢弃数据
        if(average.compareTo(new BigDecimal("200000")) == 1) {
            return true;
        }
    ...
    public static boolean isAreaScope(String area) {

        if(!MathUtil.isNumber(area)){
            return true;
        }

        if(new BigDecimal(area).compareTo(new BigDecimal("11")) == -1&&isNumeric(area)){
    ...
    public static String formatToCurl(String data) {

        try {

            if(!MathUtil.isNumber(data)){
                return data;
            }
            ...
            Double dataDou = Double.valueOf(data);

            DecimalFormat df = new DecimalFormat("#,###.00");

            return df.format(dataDou);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(isAreaScope("9"));
    }
