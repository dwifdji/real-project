package com.tzCloud.arch.core.file;

import com.tzCloud.arch.common.HttpUtils;
import com.tzCloud.arch.core.file.watermark.ImageMarkUtil;
import com.tzCloud.arch.core.file.watermark.PdfMarkUtil;
...
public class FilePathUtils {
    ...
    public static String getDictionaryPath() {
        String path;
        if (isWindowsOS()) {
            path = "D://dictionary//my";
        } else {
            path = "/tzCloud/dictionary/my";
        }
        File file = new File(path);
        if (!file.exists()) {
            file.mkdirs();//创建父级文件路径
...
//            waterMarkByImg = file.getCanonicalPath();
            waterMarkByImg = getInputPath() + "水印.png";
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
