 ...
 * 根据城市区域  获取阿里拍卖资产
 */
@Crawler(name = "getAliPmByCity")
@Component
public class GetAliPmByCityCrawler extends BaseSeimiCrawler {
    ...
    @Resource
    private AliPmZcService aliPmZcService;
    ...
    public void start(Response response) {
        ...
        try {
            List<TAliPmZcUrl>   urlList = aliPmUrlService.findAliPmZcTodoUrlList();


            for(TAliPmZcUrl cityUrl :urlList){
                ...
                params.put("assetName",cityUrl.getAssetName());

                push(Request.build( cityUrl.getReqUrl(), GetAliPmByCityCrawler::getAliZcList).setMeta(params));
    ...
    public void getAliZcList(Response response) {
        ...
        for (int i = 1; i <= page; i++) {
            ...
            push(Request.build(pageUrl, GetAliPmByCityCrawler::getList).setMeta(map));
        }
    }
    ...
    public void getList(Response response) {
        ...
        if (StringUtils.isNoneBlank(text)) {
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                push(Request.build(itemUrl, GetAliPmByCityCrawler::getDetail).setMeta(map));
    ...
    private void getExecuteDetail(Response response) {
        ...
        List<JXNode> picList = document.selN("//ul[@id='J_UlThumb']/li");
        ...
        for(JXNode tr:tableList){
            ...
            for(JXNode td: tdList){
                ...
                if("市场价".equals(tdKey)){
                    payPrice = td.sel("/span/span/text()").get(0).toString().replaceAll(":","");
                }
            }
        ...
        model.setCode(getValueByKey(reqMap,"id"));
        ...
        model.setTitle(getValueByKey(reqMap,"title"));
        model.setStatus(getValueByKey(reqMap,"status"));
        ...
        model.setStartDate(ComUtils.timeStamp2Date(getValueByKey(reqMap,"startDate")));
        model.setEndDate(ComUtils.timeStamp2Date(getValueByKey(reqMap,"endDate")));
        model.setStartStamp(getValueByKey(reqMap,"startDate"));
        model.setEndStamp(getValueByKey(reqMap,"endDate"));
        ...
        model.setMarketPrice(payPrice);
        model.setAddressDetail(StringUtils.isEmpty(addressDetail)?model.getTitle():addressDetail);

        model.setAssetName(getValueByKey(reqMap,"assetName"));


        aliPmZcService.saveAliPmZc(model);
